function SpringzillaViewModel() {
  ...
  self.submitRestoreForm = function() {
    ...
    var restoreUrl = $("#getRestoreUrl").val();
    $.ajax({
      type: "POST",
      url: restoreUrl,
      ...
      console.log("done posting data: " + data);
			if(data == "OK"){
				// Success!
			}else{
				self.error(new Error("ERROR", "Unable to start the session, server replied with: " + data));
			}
    })
  }
...
$("#restoreForm").validate({
  ...
  messages: {
    ...
    numberOfClients: {
      ...
      min: "The number of clients has to be more than 0",
    },
    maxTimeToWait: {
  ...
  submitHandler: springModel.submitRestoreForm(),
